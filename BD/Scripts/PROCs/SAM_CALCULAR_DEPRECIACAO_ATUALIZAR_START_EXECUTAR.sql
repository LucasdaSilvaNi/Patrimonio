DROP PROCEDURE SAM_CALCULAR_DEPRECIACAO_ATUALIZAR_START_EXECUTAR
GO

--CREATE PROCEDURE [dbo].[SAM_CALCULAR_DEPRECIACAO_ATUALIZAR_START_EXECUTAR] ---100112
--	@CodigoUge INT,
...
--		   AND [ast].[AssetStartId] NOT IN(SELECT [AssetStartId]  FROM [dbo].[MonthlyDepreciation] [de]
--											WHERE [de].[ManagerUnitId] = @ManagerUnitId)
...
--		BEGIN
--			SET @AssetStartId = (SELECT TOP 1 AssetStartId FROM @TableItemMaterial);
