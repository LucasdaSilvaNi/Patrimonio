DROP PROCEDURE CONSOLIDACAO_IMPORTACAO_UGE
GO

--CREATE PROCEDURE [dbo].[CONSOLIDACAO_IMPORTACAO_UGE]  --510101 -- 380251
--(    
-- --@CodOrgao int
-- @CodManagerUnit int    
--)    
...
-- DECLARE @ManagerUnitId int = (select id from ManagerUnit where Code = @CodManagerUnit)    
...
-- AND    
--  a.Id in (select AssetId from AssetMovements  with(nolock) where ManagerUnitId = @ManagerUnitId and AssetId not in (select distinct AssetId from Closing))    
-- and    
...
--   m.ManagerUnitId = @ManagerUnitId and a.Id not in (select distinct AssetId from Closing)      
...
--  flagVerificado = 2    
-- WHERE     
--  Id in (select AssetId from AssetMovements with(nolock) where ManagerUnitId = @ManagerUnitId and AssetId not in (select distinct AssetId from Closing))    
-- and    
...
--  a.Id = am.AssetId    
-- WHERE     
--  am.ManagerUnitId = @ManagerUnitId  and a.Id not in (select distinct AssetId from Closing)  
-- and    
...
--	id IN (select AssetId from AssetMovements where ManagerUnitId = @managerUnitId)
...
--   flagVerificado IS NULL AND                   
--   flagDepreciaAcumulada IS NULL AND    
--   Id in (select AssetId from AssetMovements with(nolock) where ManagerUnitId = @ManagerUnitId)
--    and  AcquisitionDate < @dateFechamentoReferencia)        
--  IF(@contador = 1)        
--  BEGIN        

--  --select 'exe'
--   EXEC [DEPRECIACAO_ACUMULADA_CARGA_UGE]  @ManagerUnitId      
--  END        
--  ELSE        
--  BEGIN         
--   SET @contador = 0        
